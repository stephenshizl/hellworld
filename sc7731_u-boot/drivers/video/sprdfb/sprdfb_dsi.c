...
{
#ifdef CONFIG_DSIH_VERSION_1P21A
	dsi_core_write_function((uint32_t)DSI_CTL_BEGIN,  (uint32_t)R_DSI_HOST_EDPI_CMD_SIZE, 0x500);
#else
	dsi_core_write_function((uint32_t)DSI_CTL_BEGIN,  (uint32_t)R_DSI_HOST_EDPI_CFG, 0x10500);
#endif
	return 0;
}

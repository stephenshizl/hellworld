...
static int sci_adc_vol_request(int channel, int scale, int mux, int* adc)
{
	...
	#else
		chan_vol = CHGMNG_AdcvalueToVoltage(chan_adc);
	#endif
...
#include <asm/arch/__sc2723_regulator_map.h>
...
int DCDC_Cal_ArmCore(void)
{
	...
	printf("%s; adie chipid:(0x%08x), mixed_ctl:(0x%08x), otp_sel:(0x%08x), cali_mask:(0x%08x), VBAT(vol %d, adc %d)\n",
	...
	printf("REG_DCDC_PD(0x%08x): (0x%08x), REG_LDO_PD(0x%08x): (0x%08x), \n",
...
#else
vol_para_t **ppvol_para = 0x50005C20; //0x50003C20
#endif

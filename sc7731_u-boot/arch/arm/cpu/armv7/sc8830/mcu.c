...
void dcdc_calibrate(int chan, int to_vol)
{
	...
	{
		if (chan == 10) { // dcdc arm
			ANA_REG_MSK_OR(ANA_REG_GLB_MP_MISC_CTRL,((dcdc_ctl_vol[i][0] & 0x7) << 6), BIT(6)|BIT(7)|BIT(8));
		}
		else if (chan == 11) {//dcdc core
			ANA_REG_MSK_OR(ANA_REG_GLB_MP_MISC_CTRL,((dcdc_ctl_vol[i][0] & 0x7) << 3), BIT(3)|BIT(4)|BIT(5));
		}
	}
	else
	{
		...
		if (chan == 10) { // dcdc arm
			ANA_REG_SET(ANA_REG_GLB_DCDC_ARM_ADI, cal_vol);
			ANA_REG_MSK_OR(ANA_REG_GLB_MP_MISC_CTRL, ((dcdc_ctl_vol[i-1][0] & 0x7) << 6), BIT(6)|BIT(7)|BIT(8));
		}
		else if (chan == 11) {//dcdc core
			ANA_REG_SET(ANA_REG_GLB_DCDC_CORE_ADI, cal_vol);
			ANA_REG_MSK_OR(ANA_REG_GLB_MP_MISC_CTRL, ((dcdc_ctl_vol[i-1][0] & 0x7) << 3), BIT(3)|BIT(4)|BIT(5));
		}
	}
#else
	if(i==0)
...
vol_para_t vol_para[7 + 1] __align(16) = {
	...
	[4] = {
		.ideal_vol = 2100,	.name = "dcdcgen",
	},
#else
	[4] = {
		.ideal_vol = 2400,	.name = "dcdcgen",
	},
#endif

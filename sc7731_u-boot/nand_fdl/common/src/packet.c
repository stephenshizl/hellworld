...
void FDL_PacketDoIdle (void)
{
    ...
    PACKET_T       *tmp_ptr    = NULL;
    uint32          crc;
    uint32 boot_mode = FDL_GetBootMode();
    unsigned char   ch;
    int ch1;
        ...
        switch (packet_ptr->pkt_state)
        {
            ...
            case PKT_GATHER:
                if (HDLC_FLAG == ch) 
				{
                    ...
                    if (0 != crc){
                        int k;
                        printf("%4x%4x", packet_ptr->packet_body.type, packet_ptr->packet_body.size);
                    ...
                    *(pdata + packet_ptr->data_size) = ch;
                    packet_ptr->data_size += 1;
                    if(boot_mode == 0x5A){ // In USB Mode
                        packet_ptr->data_size += gFdlUsedChannel->Read(gFdlUsedChannel,
                            pdata + packet_ptr->data_size, MAX_PKT_SIZE);
                        goto end;
                    }
                }
                break;

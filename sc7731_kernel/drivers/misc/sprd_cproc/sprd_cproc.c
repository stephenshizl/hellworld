...
static int sprd_cproc_write(struct file *filp, const char __user *buf,size_t count,loff_t *pos)
{
	char temp_buf[32];
	printk("%s\n",__func__);
	...
	printk("%s\n",temp_buf);
	if(strncmp(temp_buf,"WCN-CP2-EXCEPTION",17) == 0)
		flag_user = 1;
